...
exports.help = {
  ...
  usageDelim: " ",
  extendedHelp: `The conf command handles server specific settings elaborately. It lets you customize the guild settings, just like Mee6, but MUCH better. 

Each conf parameter has a specific type. admin and mod roles are roles, so they can be set by mentioning the role or through role IDs. welcomeChannel and modlogChannel can be set by #channelname. 

Conf list will list all guild settings. Conf reset will reset the specified setting. Conf set will set the parameter. 

Examples: -conf set welcomeMessage Welcome to %SERVER%,  %MEMBER%

NB: welcomeMessage posts the specified welcome message to welcomeChannel. Both parameters are required to be set for it to work. 

welcomeMessage substitutes:
